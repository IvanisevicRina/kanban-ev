...
public class MainController {
    ...
    private void showGameResult() {
        ...
        if (numberOfWinners==1){
            ...
            gameResultLabel.setText("Igra je završila izjednačeno.\nPobjedničko mjesto dijele: " + winnerNames +" sa osvojenih "+highestScore);
        }
    }
    @FXML
    ...
    private void handleLoadGame(){
            try{
                kanbanEngine = KanbanEngine.restore(fileService.loadGame());
                gameState = kanbanEngine.getGameState();
                currentPlayerIndex=kanbanEngine.findNextPlayerIndex();
                carModelComboBox.getSelectionModel().clearSelection();
                actionResultLabel.setText("Spremljena igra uspješno je učitana");
                refreshView();
            }catch (IllegalStateException e){
                actionResultLabel.setText("Učitavanj nije uspjelo" + e);
            }
    }
    @FXML
